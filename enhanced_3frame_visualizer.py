...
Key Features:
- Maximum rendering speed with optimized ffmpeg settings
- No transparency overlays for faster processing
- Efficient memory usage with on-demand overlay creation
...
def load_and_process_csv_data(csv_file: Path, start_iter: int, end_iter: int) -> List[IterationData]:
    ...
    print(f"📊 Loading data from {csv_file.name} (iterations {start_iter}-{end_iter})")
    ...
    try:
        ...
        if N * N != num_cells:
            print(f"❌ Error: Grid is not square ({num_cells} cells)")
            return []
        ...
        if df.empty:
            print(f"⚠️  No data found in iteration range {start_iter}-{end_iter}")
            return []
        ...
        print(f"✅ Successfully loaded {len(iterations)} iterations")
        return iterations
...
def create_ultra_optimized_3frame_animation(iterations: List[IterationData], 
    ...
    print(f"🎬 Creating 3-frame animation: {len(iterations)} iterations = {total_frames} frames")
    ...
    try:
        ...
        if 'ffmpeg' not in animation.writers.avail:
            print("❌ FFmpeg not available. Trying pillow...")
        ...
        # Create animation
        print(f"🎭 Setting up animation with {fps} FPS...")
        ...
        if os.path.exists(output_filename):
            ...
            print(f"⏱️  Render time: {save_time:.1f} seconds")
            print(f"🎬 {total_frames} frames at {fps} FPS = {total_frames/fps:.1f} second animation")
        else:
            print(f"❌ Failed to save animation file")
...
def batch_process_3frame_animations(
    *base_filenames: str,
    ...
    print(f"\n🎬 Starting 3-Frame Animation Batch Processing")
    print(f"📊 Parameters: Iterations {start_iteration}-{end_iteration}, {fps} FPS, {dpi} DPI")
...
def main():
    ...
    print("🎬 Ultra-Optimized 3-Frame Ising Model Animation System")
    print("=" * 60)
    ...
    config = {
        'start_iteration': 0,
        'end_iteration': 30,    # Moderate range for testing
        'fps': 8,              # Balanced speed - not too fast, not too slow
    ...
    batch_process_3frame_animations(
        *test_files,
        **config
    )

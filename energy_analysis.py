...
def energy_analysis_driver(csv_files, table_labels, curve_colours, title="Energy Analysis"):
    ...
    # Load all data files using the exact same approach as the notebook
    for csv_file, table_label, color in zip(csv_files, table_labels, curve_colours):
        ...
        try:
            ...
            # Check for energy column - match notebook's approach
            if 'Energy' in df.columns:
                ...
                print(f"[ERROR] No energy column found in {csv_file}", file=sys.stderr)
                continue
            ...
            print(f"[ERROR] Failed to process {csv_file}: {e}", file=sys.stderr)
            continue
    ...
    for label, data in all_data.items():
        ax1.plot(data['iterations'], data['energy'], 
                label=label, color=data['color'], linewidth=1.5, alpha=0.8)
    ax1.set_xlabel('Iteration')
    ax1.set_ylabel('Energy')
    ax1.set_title('Energy vs Iteration (Linear Scale)')
    ax1.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    ...
    for label, data in all_data.items():
        rolling_avg = data['energy'].rolling(window=min(100, len(data['energy'])//10), min_periods=1).mean()
        ax3.plot(data['iterations'], rolling_avg, 
                label=label, color=data['color'], linewidth=2, alpha=0.9)
    ax3.set_xlabel('Iteration')
    ax3.set_ylabel('Rolling Average Energy')
    ax3.set_title('Rolling Average Energy (Linear Scale)')
    ax3.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    ...
    for label, data in all_data.items():
        rolling_avg = data['energy'].rolling(window=min(100, len(data['energy'])//10), min_periods=1).mean()
        actual_window = min(window_1000, len(rolling_avg)//5)
        if actual_window < 10:
            actual_window = min(10, len(rolling_avg)//2)
    ...
    ax6.set_xlabel('Iteration')
    ax6.set_ylabel('|Slope of Rolling Average| (Log Scale)')
    ax6.set_title('Rolling Average Slope over ~1000 Iterations')
    ax6.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    ax6.grid(True, alpha=0.3)
    
    plt.tight_layout()
    
    # Save the plot with a simple filename based on the first file
    if table_labels:
        output_filename = f"{table_labels[0]}_energyanalysis.png"
        plt.savefig(output_filename, dpi=300, bbox_inches='tight')
        print(f"Energy analysis plot saved as: {output_filename}")
